...
def astar_path(G, source, target, weight, heuristic=None, a=1.0):
    ...
    explored = {}
    # Number of edges evaluated...for the write up
    evaluated = 0
	...
	if curnode == target:
            ...
            while node is not None:
                ...
                node = explored[node]
            path.reverse()
	    print("Number of edges evaluated by lazy A*: " + str(evaluated))
            return path
        ...
        for neighbor, w in G[curnode].items():
            if neighbor in explored:
                continue
            evaluated += 1
	    valid, ___ = weight(curnode, neighbor)
	    if(not valid):
		continue
