...
class ROSPlanner:
    def __init__(self, heuristic_func, weight_func, num_vertices, connection_radius,
        ...
        rospy.Subscriber(rospy.get_param("/pose_topic", "/car_pose"),
            PoseStamped, self.get_current_pose)
        print("Pose topic is ", rospy.get_param("/pose_topic", "/car_pose"))
	    ...
	    g_time = time.time() - g_start
            print("Graph making time: ",g_time)
        ...
        print("Set the pose of the car in rviz.")
        rospy.spin()
	...
	self.G, start_id = graph_maker.add_node(self.G, start, env=self.planning_env,
        connection_radius=self.connection_radius, start_from_config=True, lazy=True)

    	self.G, goal_id = graph_maker.add_node(self.G, goal, env=self.planning_env,
        connection_radius=self.connection_radius, start_from_config=False, lazy=True)
	lastar_start = time.time()
	path = lazy_astar.astar_path(self.G,source=start_id, target=goal_id, weight=self.weight_func, heuristic=self.heuristic_func)
    ...
    def plan_multi_goals(self, start):
        ...
        Return a path connecting them in world frame
        """
        # Implement here
        # Plan with plan_to_goal
        return world_points
...
if __name__ == '__main__':
    ...
    ROSPlanner(heuristic, weight, num_vertices=500, do_shortcut=True,
        connection_radius=100, num_goals=1)

    # This is for extra credit
    # ROSPlanner(heuristic, weight, num_vertices=500, do_shortcut=True,
    #     connection_radius=150, num_goals=3)
