...
1. You may not use code from the internet (e.g. Numpy im2col code online).
...
If you check out [nn/layers/conv_layer.py](nn/layers/conv_layer.py) you will see a pretty empty file. You should implement the direct convolution algorithm as described in class (i.e. not im2col).
...
A few hints on using Numba for this: 
- You can use Numba's `prange` to parallelize certain loops, but you should use the standard python built in `range` for spots which need accumulation (`+=`). 
- You can probably use `prange` only in the `forward` pass, but will probably need nested `range` and `prange` in the `backward`.
- If you find that sometimes your code fails, but sometimes it passes, you may have a thread safety issue from `prange`.
...
Note: you can speed up your backward pass if you save information about _where_ the max was in the forward pass.
...
The `backward` should also be quite straightforward. You can do both `forward` and `backward` in place if you choose. This should be able to take arbitrary dimensional tensors (e.g. `(b x c x h x w), (b x c x d x h x w), (b x c)...`).
...
## 7. Short answer ##
Answer these questions and save them in a file named `hw2/short_answer.pdf`.
1. See if you can improve the MNistResNetwork architecture using more ResNetBlocks. What's the highest accuracy you achieve? What is the architecture (you can paste the output from `print(network))`.
    - If you find it takes a long time to train, try substituting your own Conv and MaxPool layers with the Torch versions provided.
1. Do you get any improvement using a different non-linearity? Be sure to change it back to ReLU before you turn in your final code.
1. Can you come up with an architecture which gets even higher accuracy? Again, include the output from `print(network)`.
