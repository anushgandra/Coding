...
KM_DELTA_NOISE = 0.09 # Kinematic car delta noise std dev
...
class KinematicMotionModel:
  ...
  def apply_motion_model(self, proposal_dist, control):
    ...
    length = np.shape(proposal_dist[:,0])[0]
    
    # YOUR CODE HERE
    v = np.random.normal(v, KM_V_NOISE, length)
    delta = np.random.normal(delta, KM_DELTA_NOISE, length)
   	
    temp = proposal_dist[:,2] + ((v/self.CAR_LENGTH)*np.tan(delta)*dt)
    proposal_dist[:,0] = proposal_dist[:,0] + (float(self.CAR_LENGTH)/(np.tan(delta))*(np.sin(temp)-np.sin(proposal_dist[:,2])))
    proposal_dist[:,1] = proposal_dist[:,1] + (float(self.CAR_LENGTH)/(np.tan(delta))*(-np.cos(temp)+np.cos(proposal_dist[:,2])))
    proposal_dist[:,2] = temp
    

    
    proposal_dist[:,0] = proposal_dist[:,0] + np.random.normal(0, KM_X_FIX_NOISE, length)
    proposal_dist[:,1] = proposal_dist[:,1] + np.random.normal(0, KM_Y_FIX_NOISE, length)
    proposal_dist[:,2] = proposal_dist[:,2] + np.random.normal(0, KM_THETA_FIX_NOISE, length)    
