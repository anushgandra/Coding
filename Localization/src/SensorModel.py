...
Z_RAND = 0.5   # Weight for random reading
SIGMA_HIT = 4.5 # Noise value for hit reading
...
class SensorModel:
  ...
  def precompute_sensor_model(self, max_range_px):
    ...
    for i in range(table_width):
		...
		p_hit = (1.0/np.sqrt(2.0*np.pi*SIGMA_HIT**2.0))*np.exp(-0.5*(((i-j)**2.0)/(SIGMA_HIT**2.0)))
		if (i==max_range_px):
			p_max=1.0
		else:
			p_max=0.0
		...
		if(j>i):
			...
			p_short = 0.0
		sensor_model_table[i,j] = (((Z_HIT)/den)*p_hit) + (((Z_MAX)/den)*p_max) + (((Z_RAND)/den)*p_rand) + (((Z_SHORT)/den)*p_short)

    for k in range(table_width):
	sensor_model_table[:,k] = sensor_model_table[:,k]/float(np.linalg.norm(sensor_model_table[:,k],ord=1))
